...
useEffect(() => {
  ...
  const handleCheckCode = async () => {
    ...
    try {
      setActiveTab('output');
      setExecutingQuery(true); // Button becomes disabled here
      ...
      // const url = 'https://pyrunnerapi.azurewebsites.net/runpy/';
      setResponse(null);
      ...
      setExecutingQuery(false); // Button becomes re-enabled here
    }
  };
                                  ...
                                  {Object.entries(testCase).map(([key, value], i) => {
                                    ...
                                    return (
                                      <span
                                        ...
                                        style={{ backgroundColor: '#FFFFFF', fontSize: '14px',boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.25)', }}
                                      >
                                        {key}: <span className={textColor} style={{paddingBottom:'15%'}}>{value}</span>
                                      </span>
                                    );
                                  })}
                                </p>
