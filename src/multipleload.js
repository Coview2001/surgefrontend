...
const sidebarStyle = {
  ...
  useEffect(() => {
    const fetchCourse1 = async () => {
      try {
        setLoadingC1(true);
        const course1Response = await axios.post('https://surgebackend.azurewebsites.net/get/course1/', { StudentId: decryptedStudentId });
    ...
    const fetchCourse2 = async () => {
      try {
        setLoadingC2(true);
        const course2Response = await axios.post('https://surgebackend.azurewebsites.net/get/course2/', { StudentId: decryptedStudentId });
        // //console.log("Course 2:", course2Response);
        
        setProgressData(course2Response.data.Prograss);
        setUserName(course2Response.data.StudentName);
        setHoursSpent((parseFloat(course2Response.data.Prograss.Duration) / 3600).toFixed(2));
        setLoadingC2(false);
      } catch (error) {
        console.error('Error fetching Course 2:', error);
      }
    };
...
<div className='d-flex'>
  ...
  <div className="container-fluid mt-5 ms-3">
            ...
            {loadingC2? "" : <h3>Hello  {userName}</h3>}  
    ...
    <h5 className='border border-2 border-secondary rounded-pill shadow-lg p-2 mb-2 bg-light' style={{ width: 'fit-content' }}>
            Rank: {loadingC3? <Spinner animation="border mx-2" style={{ color: '#3F51B5' }} /> : selectedSubIndex === -1 ? ranks.Total_Rank : ranks[internship.Sub[selectedSubIndex]]}
          </h5>
    {loadingC1 ? (
